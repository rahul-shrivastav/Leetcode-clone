...
export default function Home() {
    let [pingApi, setPingApi] = useState(false);
    useEffect(() => {
        const ping = async () => {
            try {
    ...
    if (!session.data) {
        ...
        if (!localStorage.getItem("user")) {
            localStorage.setItem(
                "user",
                JSON.stringify(session.data.user.data)
            );
        }
    }
    ...
    useEffect(() => {
        const fetchproblem = async () => {
            const response = await fetch(`/api/fetchproblems`);
                    ...
                    <div className="w-full  text-white gap-0 md:gap-10 grow xl:flex-row  flex flex-col-reverse items-center justify-center bg-black">
                        ...
                        <div className=" w-[90%] border h-[85vh] py-10 xl:w-4/12 shadow-slate-900 border-slate-600 border-opacity-50 shadow-2xl flex flex-col items-center justify-center mt-20">
                            ...
                            </div>
                            {
                                //@ts-ignore
                                session.data && (
                                    <div className="m-5 font-extralight  text-4xl">
                                        {session.status === "authenticated"
                                            ? session.data.user.data.fullName
                                            : "to CodeArena"}
                                    </div>
                                )
                            }
                            ...
                            <div className="font-thin text-2xl mt-5">
                                Total Solved :{" "}
                            ...
                            <button
                                ...
                                className="mt-10 font-thin border border-violet-600 hover:bg-gray-950  px-10 py-2  w-4/5 text-[20px] rounded-sm"
                            >
                                Solve a Random Question
                            </button>
                            <button
                                disabled
                                className=" disabled:cursor-not-allowed mt-10 font-thin border border-violet-600 hover:bg-gray-950  px-10 py-2  w-4/5 text-[20px] rounded-sm"
                            >
                                Challenge a Friend
                            </button>
